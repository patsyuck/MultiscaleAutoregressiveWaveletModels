progn4a<-function(x,y=NULL,filter="haar",n=5,r=5,pmax=5,lenmin=NULL,lenmax=NULL,lfix=30,fut=10){
...
for (i in 1:((r+1)*pmax))
  q[i]<-prod(lt[,i])
#b<-as.data.frame(bm[(lenmax-lfix+1):lenmax,])
#form<-frm
#for (i in 1:((r+1)*pmax))
#  if (q[i]==1)
...
#lz<-dec$days+fut
#pr<-future(dec,q,n,r,cf,fut,lz)# prognosis!!!!!
#dev<-sd(pr)
#w<-""
#for (i in 1:((r+1)*pmax))
#  w<-paste(w,q[i],sep="")
#par(mfrow=c(1,2))
#ind<-c(1:lfix)# set of labels of the time axis for left graphics
#ymin<-min(min(dec$appr[1,(dec$days-lfix+1):dec$days]),min(fitted(mdl)))
#ymax<-max(max(dec$appr[1,(dec$days-lfix+1):dec$days]),max(fitted(mdl)))
#plot(ind,dec$appr[1,(dec$days-lfix+1):dec$days],xlab="",ylab="",ylim=c(ymin,ymax),type='l',col="red",main=paste(filter,",r=",r,",p=",pmax,",p=",w,",l=",lfix,sep=""))
